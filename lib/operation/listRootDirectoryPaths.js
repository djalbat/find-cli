"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
function _export(target, all) {
    for(var name in all)Object.defineProperty(target, name, {
        enumerable: true,
        get: Object.getOwnPropertyDescriptor(all, name).get
    });
}
_export(exports, {
    get default () {
        return listRootDirectoryPathsOperation;
    },
    get listRootDirectoryPaths () {
        return listRootDirectoryPaths;
    }
});
var _configuration = require("../configuration");
var _messages = require("../messages");
function listRootDirectoryPathsOperation(proceed, abort, context) {
    listRootDirectoryPaths();
    proceed();
}
function listRootDirectoryPaths() {
    var rootDirectoryPaths = (0, _configuration.retrieveRootDirectoryPaths)(), rootDirectoryPathsLength = rootDirectoryPaths.length, lastIndex = rootDirectoryPathsLength - 1;
    if (lastIndex === 0) {
        console.log(_messages.NO_ROOT_DIRECTORY_PATHS_MESSAGE);
    }
    rootDirectoryPaths.forEach(function(rootDirectoryPath, index) {
        if (index > 0) {
            console.log(index, rootDirectoryPath);
        }
    });
    return lastIndex;
}

//# sourceMappingURL=data:application/json;base64,